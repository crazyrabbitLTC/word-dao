...
contract("Storage", function(accounts) {
  ...
  before(async function() {
    ...
    this.wordDao = await this.project.createProxy(Storage, {
      ...
      initArgs: [config.language, config.fee]
    });
  });

  it(`Should create a ${config.language} language storage`, async function() {
    const result = await this.wordDao.methods.language().call();
    result.should.eq(config.language);
  });

  it("should create an english language wordDao proxy with a fee of 1", async function() {
    const result = await this.wordDao.methods.fee().call();
